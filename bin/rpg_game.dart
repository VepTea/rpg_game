...
class Character implements Object {
  ...
  int hp = 50; //오버라이드 됨
  int damage = 10; //오버라이드됨
  int armor = 5; //방어력 멤버 변수 추가

  Character(this.name); //생성자: 이름을 입력받아 초기화

  void showStatus() {
    print("$name용사님 - 체력: $hp, 공격력: $damage, 방어력: $armor");
    //캐릭터의 상태를 출력하는 메서드
        //호출될 위치는 RpgGame 클래스의 battle에서 player,monster 한대씩 때리고 나서 호출됨.
  }
  ...
  void attackMonster(Monster stagedMonster) {
    ...
    print("--------------------------------------------------------");
    print(
      "체력 ${stagedMonster.hp}있는 ${stagedMonster.name}에게 -$damage 만큼 피해를 줬다는거 아니겠음~?",
    );
    //이 stagedMonster는 RpgGame 클래스의 멤버변수로 선언되어 있음.
    stagedMonster.hp -= damage; //소환된 몬스터의 hp에서 캐릭터의 공격력만큼 감소시킨다.
    if (stagedMonster.hp > 0) {
      print(
        "${stagedMonster.name}의 hp가 ${stagedMonster.hp}로 날라가 버렸다는 거 아니겠음~? 침하하~",
      );
      //몬스터의 hp를 출력한다.
    } else {
  ...
  void defend(Monster stagedMonster) {
    ...
    hp += healPoint; // 몬스터의 공격력의 1~ 1/3만큼 랜덤으로 체력을 회복한다.
...
class Monster implements Object {
  ...
  late int
  damage; // 진짜 몬스터의 공격력(랜덤으로 지정됨, attackCharacter 메서드에서 매 공격시마다 새롭게 정해짐)

  Monster(this.name, this.hp, this.maxDamage); //생성자: 이름, 체력, 공격력을 입력받아 초기화
  ...
  void attackCharacter(Character player) {
    ...
    if (damage <= player.armor) {
      ...
      damage = player.armor + 1;
    } // 그냥 player의 방어력보다 +1을 더해서 설정한다.

    player.hp -= (damage - player.armor); //플레이어의 hp에서 (몬스터의 공격력-플레이어 방어력)만큼 감소시킨다.
    print("--------------------------------------------------------");
    print("아이고오~~~!!!우리 용사님이이 글쎄~ $name에게 -$damage 만큼 공격을 그냥 받으셨는데,");
    print("용사님이 몸이 딴딴해가지고 피가 -${damage - player.armor}밖에 안깎여가지고 ");
    print("피통이 ${player.hp}가 남았다는거 아니겠음~?");
  ...
  void showStatus() {
    //몬스터의 상태를 출력하는 메서드
    //호출될 위치는 RpgGame 클래스의 battle에서 player,monster 한대씩 때리고 나서 호출됨.
    print("$name - 체력: $hp, 최대공격력: $maxDamage");
  }
}
...
class RpgGame {
  ...
  void characterNaming() {
    ...
    while (true) {
      ...
      try {
        player.name = characterName; //캐릭터 이름을 입력받아서 player 객체의 name 변수에 저장
    ...
    print("--------------------------------------------------------");
    print("용사님의 존함이 ${player.name}으로 설정되었다~ 그 말이거든요잉~?");
  }
  ...
  void gameStart() {
    ...
    print("게임을 시작합니다!");
    print(
      "${player.name} - 체력: ${player.hp} , 공격력: ${player.damage} , 방어력: ${player.armor}",
    );
  }
  ...
  void getRandomMonster() {
    ...
    print(
      "소환된 몬스터: ${stagedMonster.name} - 체력: ${stagedMonster.hp}, 최대공격력: ${stagedMonster.maxDamage}",
    );
  }
  ...
  void battle() {
    ...
    // stagedMonster와 player 객체가 전투를 시작한다.어떻게?  playerTurn()과 monsterTurn()을 반복한다.
    //
    ...
    while (stagedMonster.hp > 0) {
      // 무엇을 반복할거냐면 이런 거다.
      //  player의 턴이라는 함수를 실행한다.
      playerTurn(); // 플레이어의 턴을 실행한다
      stagedMonster.attackCharacter(player); // 몬스터의 턴을 실행한다. 몬스터가 플레이어를 공격한다.
  ...
  // 	캐릭터 객체의 hp를 올려준다.
    try {
      ...
      switch (input) {
        case '1': //1을 입력받은 경우
          player.attackMonster(stagedMonster);
          // 캐릭터의 attack 메서드를 실행해서 stagedMonster의 hp를 감소시킨다.
          break; // switch문을 빠져나온다.
        case '2': //2를 입력받은 경우
          player.defend(stagedMonster); // 캐릭터의 defend 메서드를 실행한다.
          break; // switch문을 빠져나온다.
